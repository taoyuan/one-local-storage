...
describe("crossplatform local storage", () => {
  ...
  test("get/set number", async () => {
    ...
    await expect(
      cls.setNumber("myLocalStorageKey", "aaa" as any)
    ).rejects.toThrow("value is not a number");
  ...
  test("logger", async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      "trying to insert myvalue for key myLocalStorageKey"
    );
  ...
  test("exceptions", async () => {
    ...
    await expect(cls.getItem("myLocalStorageKey")).rejects.toThrow(
    ...
    await expect(cls.removeItem("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );

    await expect(cls.setJson("myLocalStorageKey", {})).rejects.toThrow(
      "custom error message"
    );

    await expect(cls.getJson("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );
    ...
    await expect(cls.getBoolean("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );

    await expect(cls.setNumber("myLocalStorageKey", 1)).rejects.toThrow(
      "custom error message"
    );

    await expect(cls.getNumber("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );
    ...
    await expect(cls.multiGet("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );

    await expect(cls.multiRemove("myLocalStorageKey")).rejects.toThrow(
      "custom error message"
    );
