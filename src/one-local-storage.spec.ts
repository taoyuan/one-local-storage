...
describe('cross platform local storage', () => {
  beforeEach(async () => {
    await cls.clear();
  });
  ...
  test('multi set/get/remove', async () => {
    ...
    const multivalue = await cls.multiGet('myLocalStorageKey', 'anotherLocalStorageKey');
  ...
  test('get/set number', async () => {
    ...
    await expect(cls.setNumber('myLocalStorageKey', 'aaa' as any)).rejects.toThrow(
  ...
  test('exceptions', async () => {
    ...
    await expect(cls.getItem('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.setItem('myLocalStorageKey', 'myvalue')).rejects.toThrow(
      'custom error message'
    );

    await expect(cls.removeItem('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.setJson('myLocalStorageKey', {})).rejects.toThrow('custom error message');

    await expect(cls.getJson('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.setBoolean('myLocalStorageKey', false)).rejects.toThrow(
      'custom error message'
    );

    await expect(cls.getBoolean('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.setNumber('myLocalStorageKey', 1)).rejects.toThrow('custom error message');

    await expect(cls.getNumber('myLocalStorageKey')).rejects.toThrow('custom error message');
    ...
    await expect(cls.multiGet('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.multiRemove('myLocalStorageKey')).rejects.toThrow('custom error message');

    await expect(cls.clear()).rejects.toThrow('custom error message');
  });
});
